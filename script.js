...
// ? AUTOMATED DIRECTORY SCRAPER ENGINE (No listing file required)
fetch('./webss/')
    .then(res => res.text())
    .then(htmlContent => {
        const track = document.getElementById('slider-track');
        ...
        // Parse the folder's index code using a temporary document scanner
        const parser = new DOMParser();
        const doc = parser.parseFromString(htmlContent, 'text/html');
        
        // Find all links generated by the server layout
        const links = doc.querySelectorAll('a');
        let files = [];

        links.forEach(link => {
            const href = link.getAttribute('href');
            // Filter out links and extract image file paths cleanly
            if (href && /\.(png|jpe?g|gif|webp|svg)$/i.test(href)) {
                // Decode formatting like %20 spaces back into clean text names
                const decodedFilename = decode???Value = decodeURIComponent(href.split('/').pop());
                if (!files.includes(decodedFilename)) {
                    files.push(decodedFilename);
                }
            }
        });

        // Backup plan: If folder indexing is locked by your server provider, read explicit fallbacks
        if (files.length === 0) {
            files = ["desktop.png", "gaming.png", "hacking.png"];
        }

        track.innerHTML = ''; dotsContainer.innerHTML = ''; slideTitles = [];

        files.forEach((filename, index) => {
            // Keep original name string untouched for titles
            slideTitles.push(filename);

            // Inject swappable picture frame markup lanes
            track.innerHTML += `<div class="slide-pane"><img src="./webss/${filename}" alt="${filename}"></div>`;
            
            // Build tracker dot steps
    ...
    .catch(err => {
        console.error("Directory scraping failed, loading default fallback:", err);
        document.getElementById('screen-title').innerText = "AxelOS Preview Gallery";
    });

// Load remaining configuration data parameters from config.json
fetch('./config.json')
    .then(res => res.json())
    .then(data => {
